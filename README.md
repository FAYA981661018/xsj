...
<head>
    ...
    <style>
        ...
        :root { --secondary-bg: #ffffff; --border-color: #e0e0e0; --text-primary: #1f1f1f; --text-secondary: #8a8a8a; --accent-color: #007bff; }
        ...
        .message-bubble { display: flex; align-items: flex-start; gap: 19px; position: relative; width: fit-content; max-width: 100%; }
...
// ▼▼▼ 请用这个新版本替换旧的 db.version().stores() ▼▼▼
db.version(17).stores({ 
    chats: '&id, isGroup, groupId', // <-- 增加 groupId 索引，方便查询
    apiConfig: '&id', 
    globalSettings: '&id', 
...
// ▼▼▼ 请用这个【终极安全版】函数，完整替换掉你旧的 exportBackup 函数 ▼▼▼
async function exportBackup() {
    try {
        ...
        const [
            ...
            db.qzoneGroups.toArray() // <-- 确保 qzoneGroups 也被导出
        ]);

        // 【核心修正】在这里对每一项数据进行安全检查和赋值
        // 如果某项数据不存在 (undefined)，就用一个安全的空值代替
        ...
        backupData.qzoneGroups = qzoneGroups || []; // <-- 确保 qzoneGroups 也被安全处理
...
// ▼▼▼ 用这个【安全可靠版】的函数，完整替换掉旧的 importBackup ▼▼▼
async function importBackup(file) {
    if (!file) return;

    // 【安全第一】导入前必须让用户确认，因为这是毁灭性的覆盖操作！
...
state.globalSettings = globalSettings || { id: 'main', wallpaper: 'linear-gradient(135deg, #89f7fe, #66a6ff)', fontUrl: '', enableBackgroundActivity: false, backgroundActivityInterval: 60 }; // 新增间隔字段，默认60秒
...
/* ▼▼▼ 用这个【最终完美对称版】函数替换你现有的 createMessageElement 函数 ▼▼▼ */
function createMessageElement(msg, chat) {
    ...
    // 【核心修改】将气泡内部的 HTML 结构简化
    bubble.innerHTML = `
        <div class="avatar-group">${avatarHtml}</div>
        <div class="content">${contentHtml}</div>
    `;

    // 【核心修改】将气泡和时间戳【作为兄弟】依次添加到父容器中
    wrapper.appendChild(bubble);
    wrapper.appendChild(timestampEl);

    addLongPressListener(wrapper, () => showMessageActions(msg.timestamp)); // 把长按事件绑定到 wrapper 上
...
// ▼▼▼ 请用这个【超级智能版】，完整替换掉旧的 triggerAiResponse 函数 ▼▼▼
                ...
                if (postId && commentText) {
                    ...
                    if (post) {
                        ...
                        if (authorId && authorId !== 'user' && authorId !== state.activeChatId && state.chats[authorId]) {
                            ...
                            console.log(`已向 '${authorChat.name}' 发送了评论通知。`);
                        }
                        await renderQzonePosts();
                    } else {
...
function runBackgroundSimulationTick() {
    ...
    // 2. 遍历每个非激活角色，给他们一个行动的机会
    inactiveChats.forEach(chat => {
        // 设定一个概率，比如 20% 的几率行动，避免消息轰炸
        if (Math.random() < 0.20) { 
...
// ▼▼▼ 请用这个【同样智能的】，完整替换掉旧的 triggerInactiveAiAction 函数 ▼▼▼
    ...
    try {
        ...
        if (action.type === 'qzone_comment') {
            ...
            if (post) {
                ...
                updateUnreadIndicator(unreadPostsCount + 1);
                console.log(`角色 "${chat.name}" 评论了动态 #${post.id}`);
            }
            ...
            if (post) {
                ...
                if (!post.likes.includes(chat.name)) {
                    ...
                    updateUnreadIndicator(unreadPostsCount + 1);
                    console.log(`角色 "${chat.name}" 点赞了动态 #${post.id}`);
                }
            }
            ...
            renderChatList();
            console.log(`角色 "${chat.name}" 主动发送了消息: ${aiMessage.content}`);
        }
...
function applyScopedCss(cssString, scopeId, styleTagId) {
    ...
    // --- 【核心修正】使用更强大的正则表达式 ---
    // 这个正则表达式能够更好地处理复杂的选择器，包括伪类、属性选择器等
    // 它会匹配所有不在 @...{} 块内，且在 { 符号之前的部分
    let scopedCss = '';
    // 移除注释，防止干扰
    cssString = cssString.replace(/\/\*[\s\S]*?\*\//g, '');

    // 使用一个简单的解析器来处理 @ 规则和普通规则
...
// ▼▼▼ 请用这个【修正后】的函数，替换旧的 saveEditedMessage ▼▼▼
/**
 * 保存编辑后的消息，更新数据库和UI
 * @param {number} timestamp - 【核心修复】直接接收要修改的消息时间戳
 * @param {string} newRawContent - 从编辑框获取到的新内容
 */
async function saveEditedMessage(timestamp, newRawContent) {
    if (!timestamp) return; // 使用传入的时间戳进行判断
...
state.chats[newChatId] = newChat; await db.chats.put(newChat); renderChatList(); } });
            document.getElementById('add-group-chat-btn').addEventListener('click', async () => { const numStr = await showCustomPrompt('创建群聊', '请输入群成员数量 (2-20)'); const num = parseInt(numStr); if (!num || num < 2 || num > 20) { alert('请输入2到20之间的有效数字！'); return; } const name = await showCustomPrompt('设置群名', '请输入群聊的名字'); if (name && name.trim()) { const newChatId = 'group_' + Date.now(); const members = []; for (let i = 1; i <= num; i++) members.push({ id: `member_${i}`, name: `成员${i}`, avatar: defaultGroupMemberAvatar, persona: '一个路过的群成员。', avatarFrame: '' }); 
const newGroupChat = { 
    id: newChatId, 
    name: name.trim(), 
    isGroup: true, 
...
document.getElementById('create-post-btn').addEventListener('click', async () => {
    ...
    modal.querySelector('.post-mode-switcher').style.display = 'flex'; // 用flex恢复
...
// ▼▼▼ BUG修复：修正了 handleSwipeEnd 的调用方式 ▼▼▼
...
const handleSwipeEnd = (e) => { 
    ...
    // 【核心修复】从事件对象 e 中获取最终的X坐标，而不是依赖 state
    // 注意：touchend 事件需要用 changedTouches
